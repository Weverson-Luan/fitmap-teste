...
- **Aliases de import**: use os aliases configurados em vez de import relativo para tudo que não seja um arquivo vizinho (mesma pasta ou uma acima/abaixo). Aliases disponíveis: `@modules/*` → `src/modules/*`, `@shared/*` → `src/shared/*`, `@assets/*` → `src/assets/*`, `@presentation/*` → `src/shared/presentation/*`, `@store/*` → `src/shared/store/*`, `@data/*` → `src/shared/data/*`, `@libs/*` → `src/shared/libs/*`, `@styles/*` → `src/shared/styles/*`, `@utils/*` → `src/shared/utils/*`, `@config/*` → `src/shared/config/*`, `@types/*` → `src/shared/@types/*`, `@common/*` → `src/shared/common/*`. Configurados em três lugares que precisam ficar em sincronia: `tsconfig.json` (`compilerOptions.paths`), `babel.config.js` (`babel-plugin-module-resolver`, usado também pelo Metro) e `jest.config.js` (`moduleNameMapper`). Um novo alias exige atualizar os três. Dentro de um módulo, imports para arquivos do próprio módulo (`views → view-model → infra/repositories`) continuam relativos — os aliases acima são para cruzar módulo/shared, não para navegação interna.
